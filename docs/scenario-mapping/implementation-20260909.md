...
入口更新：`/prototype/index.html?locale=en-US&studio=fulljourney` 直接显示 Journey Map；地图上方仅保留左对齐的 Customer journey / Hero case 两个 tab；原目录切换已按用户后续要求移除，来源映射继续在场景弹窗中展示。用户后续要求去掉欢迎横幅和介绍卡片，现已移除，旧目录链接首次打开也先展示地图。
