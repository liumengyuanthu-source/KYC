...
function sourceItem(s, code, related) {
  ...
  return `<details class="sm-source" data-source-code="${escape(code)}" data-source-role="${related ? 'related' : 'primary'}">
    ...
    <div class="sm-source-detail"><p class="sm-role">${related ? 'Related subprocess · context, dependency or reuse' : 'Primary subprocess · scenario backbone'}</p>
...
export function scenarioMappingHtml(id) {
  ...
  return `<section class="sm-mapping" aria-labelledby="sm-mapping-title">
    <header class="sm-mapping-heading"><div><span class="sm-kicker">SCENARIO SCOPE</span><h3 id="sm-mapping-title">Subprocess mapping</h3><p>Select a subprocess to inspect its origin.</p></div><span class="sm-count"><b>${s.primary.length}</b> <span>primary</span><i> / </i><b>${s.related.length}</b> <span>related</span></span></header>
    ...
    <aside class="sm-related"><h4>Related <span>Context & dependencies</span></h4>${s.related.length ? s.related.map(code => sourceItem(s, code, true)).join('') : '<p class="sm-none">No related subprocesses specified.</p>'}<p class="sm-related-note">These subprocesses provide inputs, reuse links or handoffs. They are not additional sequential subprocesses.</p></aside></div>
    <details class="sm-reading-note"><summary>How these subprocesses shape the scenario</summary><p>${escape(s.rationale)}</p><p>${escape(s.note)}</p><p>Subprocess labels show provenance, not an execution order. Detailed workflow changes remain proposals for discussion.</p></details>
...
export function mappingDirectoryHtml(stage = 'all', query = '') {
  ...
  return `<section class="sm-directory" aria-label="Scenario subprocess mapping directory">
    <div class="sm-directory-head"><div><h2>Your scenario map</h2><p>Choose a scenario to explore its primary and related subprocesses.</p></div><label class="sm-search"><span class="sr-only">Search scenarios or subprocesses</span><input type="search" data-sm-search value="${escape(query)}" placeholder="Scenario, M0.1 or C2.7" aria-label="Search scenarios or subprocesses"></label></div>
    ...
    <div class="sm-directory-results"><p class="sm-results" role="status"><b>${hits.length}</b> <span>scenarios shown</span></p><div class="sm-table-wrap"><table class="sm-table"><thead><tr><th>Scenario</th><th>Primary subprocesses</th><th>Related subprocesses</th><th><span class="sr-only">Open scenario</span></th></tr></thead><tbody>${hits.map(s => `<tr><th scope="row"><span class="sm-scenario-number">${s.number}</span><button type="button" data-sn="scene" data-sn-value="${s.id}">${escape(s.title)}</button><small>${s.stage==='Clear'?'Confirm readiness':s.stage}</small></th><td><span class="sm-mobile-label">Primary subprocesses</span>${s.primary.map(code => `<code>${code}</code>`).join(' ')}</td><td><span class="sm-mobile-label">Related subprocesses</span>${s.related.length ? s.related.map(code => `<code>${code}</code>`).join(' ') : '—'}</td><td><button type="button" class="sm-open" data-sn="scene" data-sn-value="${s.id}" aria-label="${escape('Open '+s.number+' '+s.title)}">View mapping <span aria-hidden="true">→</span></button></td></tr>`).join('')}</tbody></table>${hits.length ? '' : '<p class="sm-empty">No scenarios match. Try another subprocess or select All scenarios.</p>'}</div></div>
  </section>`;
}
...
export function scenarioJourneyProcessMatrixHtml() {
  return `<section class="sm-four-level" aria-labelledby="sm-four-level-title">
    <div class="sm-four-level-heading"><h3 id="sm-four-level-title">How the four levels map</h3><p>The journey stage shows when the scenario is discussed. Process groups organise the work. M/C subprocesses provide exact traceability.</p></div>
    <div class="sm-table-wrap"><table class="sm-table sm-four-level-table"><thead><tr><th>Customer journey</th><th>Process group</th><th>Scenario</th><th>Subprocesses</th></tr></thead><tbody>
      ${scenarioMappings.map(s => `<tr data-four-level-scenario="${escape(s.id)}"><td><span class="sm-mobile-label">Customer journey</span><strong>${s.stage === 'Clear' ? 'Confirm readiness' : s.stage}</strong></td><td><span class="sm-mobile-label">Process group</span>${processGroupChips(s.primary,'Primary')}${processGroupChips(s.related,'Related')}</td><th scope="row"><span class="sm-mobile-label">Scenario</span><span class="sm-scenario-number">${s.number}</span><button type="button" data-sn="scene" data-sn-value="${escape(s.id)}">${escape(s.title)}</button></th><td><span class="sm-mobile-label">Subprocesses</span>${sourceActionChips(s.primary,'Primary')}${sourceActionChips(s.related,'Related')}</td></tr>`).join('')}
    </tbody></table></div>
  </section>`;
}
...
export function scenarioSourceIndexHtml() {
  return `<details class="sm-scenario-index-fold">
    <summary><span><strong>Scenario list & subprocess mapping</strong><small>See how S1–S15 map to M/C subprocesses.</small></span><span class="sm-fold-count">15 scenarios <b aria-hidden="true">+</b></span></summary>
    <div class="sm-scenario-index-body">${scenarioJourneyProcessMatrixHtml()}<h3 class="sm-source-list-title">Scenario subprocess details</h3><p class="sm-scenario-index-note">Primary subprocesses form the scenario backbone. Related subprocesses provide inputs, reuse or handoffs.</p>
      <div class="sm-table-wrap"><table class="sm-table"><thead><tr><th>Scenario</th><th>Primary subprocesses</th><th>Related subprocesses</th><th><span class="sr-only">Open scenario</span></th></tr></thead><tbody>
        ${scenarioMappings.map(s => `<tr data-scenario-id="${escape(s.id)}"><th scope="row"><span class="sm-scenario-number">${s.number}</span><button type="button" data-sn="scene" data-sn-value="${escape(s.id)}">${escape(s.title)}</button><small>${s.stage === 'Clear' ? 'Confirm readiness' : s.stage}</small></th><td><span class="sm-mobile-label">Primary subprocesses</span>${s.primary.map(code => `<code>${code}</code>`).join(' ')}</td><td><span class="sm-mobile-label">Related subprocesses</span>${s.related.length ? s.related.map(code => `<code>${code}</code>`).join(' ') : '—'}</td><td><button type="button" class="sm-open" data-sn="scene" data-sn-value="${escape(s.id)}" aria-label="${escape(`Open ${s.number} ${s.title}`)}">Open mapping <span aria-hidden="true">→</span></button></td></tr>`).join('')}
      </tbody></table></div>
    </div>
  </details>`;
}
