...
import {activityById,journeyRoles,processes} from './journey/model.mjs';
...
export const dialogScrollKey=state=>state.reference?`reference:${state.reference.scene||'all'}:${state.reference.node||'all'}`:state.journeyView==='activity'&&state.journeyActivity?`activity:${state.journeyActivity}`:state.reconstruction&&pc01Scenes.includes(state.scenario)?`scene:${state.scenario}:${state.reconstruction.panel}:${state.reconstruction.mappingId||'overview'}`:`scene:${state.scenario}`;
// Resolve from the visible reader before print mode closes its modal. Last-scene
// context alone must not turn a full Journey print into a specialized scene print.
export function journeyPrintScope(state){
 if(state.page!=='studio'||state.studioPage||state.reference||state.branchIndex||state.reconstructionPrint)return null;
 if(!state.modal)return 'all';
 return state.journeyView==='activity'&&activityById(state.journeyActivity)?'activity':null;
}
export function initialNavigation() { return {journeyRole:'ROLE-KYCOPS',journeyActivity:null,journeyProcess:'M0',journeyView:null,page:'studio',mode:'story',locale:'en-AU',comparison:'target',trigger:'new-relationship',role:'ROLE-KYCOPS',stage:'S1',scenario:'SCN-SCOPE',modal:false,storyCursor:0,routeId:'ROUTE-NEW-REL-01',camera:{anchor:'SCN-SCOPE',offset:0},zoom:'read',step:'requirements',returnToken:null,graph:{scene:null,focus:null,branch:null},dGraph:{graph:'DG-D01',focus:null,cursor:0,playing:false},specialistBeat:null,clearanceBeat:null,eFocus:0,conditionDomain:'conflicts',agreementRevision:null,liveAgreementRevision:null,liveCaseRevision:null,dependency:null,reference:null,productScroll:0,transformation:{compare:false,selectedChange:null,openChanges:[],diagramFocus:null}}; }
export function navigate(state, a) {
 const s=structuredClone(state);
 if(['SCENE','RECONSTRUCTION_SCENE','CONTINUE','RESUME','PREVIOUS','D_BEAT','E_BEAT'].includes(a.type)){s.journeyActivity=null;s.journeyView=null;}
 ...
 switch(a.type){
 case 'JOURNEY_OPEN':s.page='studio';s.studioPage=null;s.modal=false;s.mode='explore';s.reference=null;s.branchIndex=false;s.reconstructionPrint=false;s.journeyPrint=null;s.journeyView=null;break;
 case 'JOURNEY_ROLE':if(a.value==='all'||journeyRoles.some(r=>r.id===a.value)){s.journeyRole=a.value;s.journeyActivity=null;s.journeyView=null;s.modal=false;}break;
 case 'JOURNEY_PROCESS':{const p=processes.find(p=>p.id===a.value);if(p){s.journeyProcess=p.id;s.camera={anchor:p.id,offset:0};s.stage=p.stage;s.zoom='read';}break;}
 case 'JOURNEY_ACTIVITY':{const activity=activityById(a.id);if(!activity)break;const detail=navigate(s,{type:'SCENE',id:activity.scene});Object.assign(s,detail,{journeyRole:s.journeyRole||'ROLE-KYCOPS',journeyProcess:activity.process,journeyActivity:activity.id,journeyView:'activity',reference:null,modal:true});break;}
 case 'JOURNEY_DETAIL':if(activityById(s.journeyActivity)){s.journeyView='detail';s.modal=true;}break;
 ...
 case 'DIALOG_ESCAPE':return navigate(s,{type:s.reference?'REFERENCE_CLOSE':s.journeyView==='activity'?'CLOSE':s.branchIndex?'CLOSE':pc01Scenes.includes(s.scenario)&&s.reconstruction?.panel?'RECONSTRUCTION_ESCAPE':'CLOSE'});
 ...
 case 'RETURN':{
  ...
  return {...origin,page:'studio',modal:a.destination==='scenario',locale:s.locale,returnToken:null,originScopeRevision:s.originScopeRevision,...(origin.journeyActivity?{journeyView:'activity'}:{})};
 }
