...
import {clearanceReferences} from './references/clearance.mjs';
import {withConsolidationBindings} from './references/consolidation.mjs';
...
const mergeBindings=cards=>{const merged=new Map();for(const card of cards){const key=card.source.source_id+'|'+card.observation.observation_id,prior=merged.get(key);if(prior)merged.set(key,{...prior,binding:{...prior.binding,scenario_refs:[...new Set([...prior.binding.scenario_refs,...card.binding.scenario_refs])],semantic_node_refs:[...new Set([...prior.binding.semantic_node_refs,...card.binding.semantic_node_refs])]}});else merged.set(key,card);}return [...merged.values()];};
export function selectReferences({scene=null,node=null,kind=null,query='',selected=null}={}){
 const q=query.trim().toLowerCase();
 return mergeBindings([...registry.bindings.map(binding=>({binding,source:registry.sources.find(s=>s.source_id===binding.source_ref),observation:registry.observations.find(o=>o.observation_id===binding.observation_ref),interpretation:registry.interpretations.find(i=>i.interpretation_id===binding.interpretation_ref)})),...designReferences,...projectCards,...clearanceReferences]).map(withConsolidationBindings).filter(x=>(!scene||x.binding.scenario_refs.includes(scene))&&(!node||x.binding.semantic_node_refs.includes(node))&&(!kind||x.source.source_kind===kind)&&(!selected||x.observation.observation_id===selected)&&(!q||JSON.stringify(x).toLowerCase().includes(q)));
}
