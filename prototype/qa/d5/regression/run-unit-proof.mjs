...
writeFileSync('prototype/qa/d5/regression/unit-results.tap',run.stdout+run.stderr);
...
writeFileSync('prototype/qa/d5/regression/unit-results.json',JSON.stringify({at:new Date().toISOString(),command:'node --test --test-reporter=tap prototype/tests/*.test.mjs',exit_code:run.status,tests,hashes:Object.fromEntries(files.map(f=>[f,createHash('sha256').update(readFileSync(f)).digest('hex')]))},null,2)+'\n');
