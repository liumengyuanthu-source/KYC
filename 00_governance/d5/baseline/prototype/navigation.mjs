...
export function navigate(state, a) {
 ...
 case 'SCENE':{const beat=a.beat||specialistBeatForScene(a.id);s.branchIndex=false;s.reconstructionTrail=[];s.scenario=a.id;s.stage=a.stage||stageFor(a.id);s.specialistBeat=beat;s.dGraph=synchronizeSpecialistGraph(s,beat);s.reconstruction=readerState(a.id);s.modal=true;break;}
 ...
 case 'RESUME':s.branchIndex=false;s.reconstructionTrail=[];s.scenario=route[s.storyCursor];s.specialistBeat=specialistBeatAt(s.storyCursor);s.dGraph=synchronizeSpecialistGraph(s,s.specialistBeat);s.stage=stageFor(s.scenario);s.camera={anchor:s.scenario,offset:0};s.mode='story';s.modal=true;break;
 case 'PREVIOUS':s.branchIndex=false;s.reconstructionTrail=[];s.storyCursor=Math.max(0,s.storyCursor-1);s.scenario=route[s.storyCursor];s.specialistBeat=specialistBeatAt(s.storyCursor);s.dGraph=synchronizeSpecialistGraph(s,s.specialistBeat);s.stage=stageFor(s.scenario);s.camera={anchor:s.scenario,offset:0};break;
 }
