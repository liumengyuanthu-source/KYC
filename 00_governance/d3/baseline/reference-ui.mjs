...
export function referenceHtml({locale='en-AU',scene=null,node=null,kind=null,query='',selected=null,print=false}={}){
 ...
 const articles=cards.map(({source:s,observation:o,interpretation:i,binding:b})=>{
  ...
  return `<article class="reference-card" id="ref-${esc(o.observation_id)}" data-reference="${esc(o.observation_id)}">
   <small>${esc(o.observation_id)} · ${esc(s.source_kind)}</small><h3>${esc(s.title)}</h3>
   ...
   <p>${esc(zh?i.why_it_matters:'Why here: '+(b.scenario_refs.join(' / ')||'Studio interaction')+' · '+b.semantic_node_refs.join(' / '))}</p>
   <dl><dt>Adopt</dt><dd>${esc(zh?i.adopt:isDesign?i.adopt:'Use the bounded mechanism described above as a design reference.')}</dd><dt>Adapt</dt><dd>${esc(zh?'使用本案对象、用途和版本；权限与银行适用性另行确认。':i.adapt)}</dd><dt>Avoid</dt><dd>${esc(zh?i.avoid:isDesign?i.avoid:'Do not transfer market-specific permissions, bank policy or unverified product capabilities. Retain the source limits above.')}</dd></dl>
   <p class="reference-status">${esc(zh?o.verification_status:isDesign?'Approved demo design only':isProject?o.verification_status:'Verification status inherited from research; see original wording')} · ${provenance} · ${t('无新增核验日期','No new verification date')}</p>
   ${!isDesign&&!isProject?`<p>${t('媒体未取得：不展示虚构截图。','Not captured: no fabricated screenshot preview.')}</p>`:''}${original}${link}
   ${!print&&b.scenario_refs[0]?`<button data-ref-scene="${esc(b.scenario_refs[0])}">${t('前往关联场景','Go to linked scenario')}</button>`:''}<small>${esc(b.support_kind)} · ${esc(b.semantic_node_refs.join(' / '))}</small></article>`;
 }).join('');
