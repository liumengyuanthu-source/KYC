...
const rows=(items,locale)=>items.map(([name,value])=>`<div class="d-detail-row"><dt>${esc(name)}</dt><dd>${Array.isArray(value)?value.map(x=>`<span class="d-token">${esc(prose(x,locale))}</span>`).join(' '):esc(prose(value??pair(locale,'Not recorded','未记录'),locale))}</dd></div>`).join('');
...
export function specialistSceneHtml({scene='SCN-CONFLICTS',beat='D1',mode='Target',projection={},locale='en-AU',active=false,graphState={},print=false}={}){
 ...
 const titles={D1:['Parallel work, one shared case','并行工作，同一案件'],D2:['Conflicts starts early and stays bounded','利益冲突尽早启动且边界清晰'],D3:['Credit produces a versioned condition','信贷生成版本化条件'],D4:['Legal consumes a named Credit input','法务使用明确版本的信贷输入'],D5:['Readiness keeps every unresolved condition visible','就绪状态保留每项未决条件']};
 const title=titles[beat]||titles.D1;
 ...
 const activeWork=beat==='D1'?pair(locale,'Conflicts, Credit and Legal advance on their own prerequisites.','利益冲突、信贷与法务依据各自前置条件推进。'):beat==='D2'?pair(locale,'Search is complete; a specialist reviews the open finding.','搜索已完成；专业人员复核未决发现。'):beat==='D3'?pair(locale,'Track CC-01 and pass a revision-addressed AI-01 to Legal.','跟踪 CC-01，并向法务传递带版本的 AI-01。'):beat==='D4'?pair(locale,'Keep AGR-01 revision 03 frozen while checking whether its Credit input remains current.','冻结查看 AGR-01 版本 03，同时检查其信贷输入是否仍为当前版本。'):pair(locale,'Inspect every unresolved condition and the authority boundary.','检查每项未决条件及权限边界。');
 const wait=beat==='D2'?pair(locale,'Configured Conflicts review and scoped hold resolution.','等待已配置的利益冲突复核及限定 Hold 解决。'):beat==='D3'?pair(locale,'Documentation incorporation before execution.','等待签署前纳入文件条件。'):beat==='D4'?pair(locale,'Validated signatory authority and execution; current Credit input if revised.','等待经验证的签署权限与签署；如信贷已修订，还需当前输入。'):pair(locale,'Conflicts review, Legal execution, QA and unresolved C conditions.','等待利益冲突复核、法务签署、QA 及 C 阶段未决条件。');
 const output=beat==='D3'?pair(locale,'A versioned Credit condition and agreement input, not case clearance.','形成版本化信贷条件与协议输入，不代表案件准入。'):beat==='D4'?pair(locale,'An internally approved agreement remains pending execution.','内部已批准的协议仍等待签署。'):pair(locale,'A scoped contribution to a still Not Ready case.','对仍为尚未就绪案件的一项限定贡献。');
...
export function specialistConditionHtml({projection={},domain='conflicts',role='ROLE-KYCOPS',locale='en-AU',agreementRevision=null,liveAgreementRevision=null,rationale='',error='',active=false,print=false}={}){
 if(!projection.available)return `<section class="d-workspace d-unavailable"><span class="eyebrow">BATCH D · ${pair(locale,'READ-ONLY PREVIEW','只读预览')}</span><h2>${pair(locale,'Condition Detail','条件详情')}</h2><p>${pair(locale,'This role or case does not have an active specialist projection. Load the authored D entry explicitly or choose a configured safe view.','此角色或案件没有可用的专业条线投影。请明确载入编制的 D 入口，或选择已配置的安全视图。')}</p></section>`;
 ...
 return `<section class="d-workspace" data-d-active="${active}"><header class="d-workspace-head"><span class="eyebrow">BATCH D · SRC-019 · ${active?pair(locale,'ACTIVE SYNTHETIC SESSION','活动中的合成会话'):pair(locale,'AUTHORED PREVIEW','编制预览')}</span><h2>${pair(locale,'Condition Detail','条件详情')}</h2><p>${esc(projection.caseId)} · ${esc(projection.scopeId)} · ${pair(locale,'Case revision','案件版本')} ${esc(projection.revision)} · ${label(projection.readiness?.result,locale)}</p></header>${controls}<article class="d-condition-card" data-d-selected="${esc(domain)}"><h3>${label(domain,locale)}</h3><dl>${rows(conditionRows,locale)}</dl></article>${conflict}${revision}<section class="d-dependencies"><h3>${pair(locale,'Dependency views','依赖视图')}</h3>${print?'':`<div class="controls"><button data-action="d-dependency" data-value="${domain==='conflicts'?'DG-D01':'DG-D02'}">${pair(locale,'View dependency','查看依赖')} · ${domain==='conflicts'?'DG-D01':'DG-D02'}</button><button data-action="d-dependency" data-value="DG-D03">${pair(locale,'Scoped Holds','限定 Holds')} · DG-D03</button></div>`}</section>${facilitator}</section>`;
}
...
export function specialistLegacyBannerHtml({locale='en-AU',canRestore=false}={}){
 return `<div class="d-legacy-guard" role="status"><div class="d-guard-line"><strong>${pair(locale,'Batch D specialist session is active · A–C writers paused','Batch D 专业条线会话已启用 · A–C 写入已暂停')}</strong>${canRestore?`<button data-action="d-restore">${pair(locale,'Restore pre-D session','恢复 D 前会话')}</button>`:''}</div><details><summary>${pair(locale,'Why are earlier writers paused?','为何暂停早期写入？')}</summary><p>${pair(locale,'Earlier A–C views remain readable, but their writers were not designed to preserve D dependencies. Restore the archived pre-D session to resume those actions.','A–C 早期视图仍可阅读，但其写入功能无法保证 D 依赖不被改写。如需恢复这些操作，请还原已归档的 D 前会话。')}</p></details></div>`;
}
